...
actual=$(curl -X PUT -s -w "%{http_code}" -o $outfile -H "Content-Length:" localhost:$port/$infile)
if [[ $actual -ne $expected ]]; then
    echo -e "Bad PUT Test: ${RED}Fail${NC}"
    kill -9 $pid
    wait $pid
    rm -f $infile $outfile
    exit 1
...
actual=$(printf "PUT /$infile HTTP/1.1\r\nContent-Length: 10" | nc localhost $port | awk '{print $2}' |grep $expected)
if [[ $actual -ne $expected ]]; then
    echo -e "Bad PUT Test: ${RED}Fail${NC}"
    kill -9 $pid
    wait $pid
...
echo -e "Bad PUT Test: ${GREEN}Pass${NC}"
kill -9 $pid
wait $pid
rm -f $infile $outfile
exit 0